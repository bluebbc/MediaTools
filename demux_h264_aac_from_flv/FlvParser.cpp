...
int CFlvParser::DumpFlv(const std::string &path)
{
	...
	{
		mySize += 15;
		mySize += (*it_tag)->_header.nDataSize;
        ...
        if ((*it_tag)->_header.nType == 0x09 && *((*it_tag)->_pTagData + 1) == 0x01) {
            ...
            */
            ...
            int i;
			int ttt = (*it_tag)->_header.nDataSize - 5 - _nNalUnitLength - 4;
			...
			//duplicate = false;
            if (duplicate) {
				int *pp = &((*it_tag)->_header.nDataSize);
				ofs << 11 << " " << (*it_tag)->_header.nDataSize << " ";
                nalu_len -= i;
				...
				SizeOyO += pStartCode - (*it_tag)->_pTagData;
				int tmp1 = pStartCode - (*it_tag)->_pTagData;
                ...
                */
				SizeOyO += (*it_tag)->_header.nDataSize - (pStartCode - (*it_tag)->_pTagData);
				int tmp2 = (*it_tag)->_header.nDataSize - (pStartCode - (*it_tag)->_pTagData);
                ...
                */
				ofs << 11 << " " << (*it_tag)->_header.nDataSize << " ";
				ofs << 11 << " "<< tmp1 <<" " << tmp2 << std::endl;
            } else {
				SizeOyO += 11;
                f.write((char *)(*it_tag)->_pTagHeader, 11);
				SizeOyO += (*it_tag)->_header.nDataSize;
                f.write((char *)(*it_tag)->_pTagData, (*it_tag)->_header.nDataSize);
				ofs << 11 << " " << (*it_tag)->_header.nDataSize << std::endl;
            }
        } else {
			SizeOyO += 11;
            f.write((char *)(*it_tag)->_pTagHeader, 11);
...
CFlvParser::Tag *CFlvParser::CreateTag(unsigned char *pBuf, int nLeftLen)
{
	...
	case 0x09:
		...
		cout << "video TS : " << header.nTotalTS << endl;
		break;
	case 0x08:
		pTag = new CAudioTag(&header, pBuf, nLeftLen, this);
		cout << "audio TS : " << header.nTotalTS << endl;
		break;
	default:
		pTag = new Tag();
